...
export default function ReportsPage() {
  ...
  function applyPreset(preset) {
    ...
    switch (preset) {
      ...
      case 'last_week': {
        ...
        setFromDate(lastMonStr);
        setToDate(getSunday(lastMonStr));
        break;
      }
      case 'this_month': {
        setFromDate(getMonthStart(todayStr));
        setToDate(todayStr);
        break;
      }
      case 'last_month': {
  ...
  if (!report) return <div className="text-center py-12">Loading...</div>;
    ...
    <div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {isRange && (
            <span className="text-xs text-gray-400 ml-2">
              ({Math.round((new Date(toDate) - new Date(fromDate)) / 86400000) + 1} {t('date', lang)})
            </span>
          )}
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          <p className="text-sm text-gray-500">{isRange ? t('sales_period', lang) : t('today_sales', lang)}</p>
          <p className="text-2xl font-bold text-primary mt-1">{formatCurrency(report.daySales.total, 'LAK')}</p>
          <p className="text-xs text-gray-400">{report.daySales.count} {t('orders', lang)}</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          <p className="text-sm text-gray-500">{t('month_sales', lang)}</p>
          <p className="text-2xl font-bold text-secondary mt-1">{formatCurrency(report.monthSales.total, 'LAK')}</p>
          <p className="text-xs text-gray-400">{report.monthSales.count} {t('orders', lang)}</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          <p className="text-sm text-gray-500">{t('gross_profit', lang)}</p>
          <p className="text-2xl font-bold text-green-600 mt-1">{formatCurrency(profit.gross_profit || 0, 'LAK')}</p>
          <p className="text-xs text-gray-400">{t('margin', lang)}: {marginPct}%</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          <p className="text-sm text-gray-500">{t('items_sold', lang)}</p>
          <p className="text-2xl font-bold mt-1">{profit.items_sold || 0}</p>
          <p className="text-xs text-gray-400">{t('cost_of_goods', lang)}: {formatCurrency(profit.cost_of_goods || 0, 'LAK')}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {/* Top Products */}
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          <h3 className="font-bold mb-3">{t('top_products', lang)}</h3>
              ...
              {report.topProducts.map((p, idx) => (
                <div key={idx} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                  ...
                  <div className="text-right">
                    <p className="font-bold text-sm">{p.total_qty} {t('items_sold', lang)}</p>
                  </div>
                </div>
              ))}
            </div>
